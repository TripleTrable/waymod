...
# o Look for make include files relative to root of waymod src
...
# turn into waymod), we will call a sub make in that other dir, and
...
# ===========================================================================
# Build targets only - this includes waymod, arch specific targets, clean
...
# Defaults to waymod, but the arch makefile usually adds further targets
all: waymod
...
waymod-dirs	:= $(objs-y)
waymod-objs	:= $(patsubst %,%/built-in.o, $(objs-y))
...
waymod: $(waymod-all) FORCE
	+$(call if_changed,waymod)

# The actual objects are generated when descending,
# make sure no implicit rule kicks in
$(sort $(waymod-all)): $(waymod-dirs) ;

# Handle descending into subdirectories listed in $(waymod-dirs)
...
define filechk_version.h
	(echo \#define WAYMOD_VERSION_CODE $(shell                         \
	expr $(VERSION) \* 65536 + 0$(PATCHLEVEL) \* 256 + 0$(SUBLEVEL)); \
	echo '#define WAYMOD_VERSION(a,b,c) (((a) << 16) + ((b) << 8) + (c))';)
endef
...
clean-dirs      := $(addprefix _clean_, . $(waymod-dirs))
...
help:
	...
	@echo  '* waymod		  - Build the application'
